+++
date = '2025-10-28T09:53:15+08:00'
draft = false
title = 'emoji表情大全'
tags = ['emoji','github']
slug = 'emoji'
...
Emoji（中文常称“绘文字”或“表情符号”）是一种用于电子通信的图形符号。它们可以表示表情、物品、符号、动物、食物、地点、旗帜等各种概念。

简单来说，**Emoji 就是数字时代的象形文字**，让沟通更生动、有趣和高效。

---

### Emoji 的核心特点：

1.  **图形化：** 它们是图片，而不是文字。这使得它们能跨越语言障碍，不同母语的人也能理解其基本含义。

2.  **标准化：** 全球有一个统一的组织——**Unicode Consortium**（统一码联盟）——负责审核和制定Emoji标准。这意味着同一个Emoji代码（如 U+1F600）在苹果、谷歌、微软、三星等不同公司的设备上显示的样子可能略有不同，但代表的是同一个概念。

3.  **情感表达：** 最初和最主要的用途是补充文字难以传达的语气和情感，比如用 😊 表示友好，用 😂 表示大笑，避免误解。

4.  **多功能性：** 除了表情，它们还可以代替文字（如用 🍕 代替“披萨”）、装饰信息，甚至讲述一个完整的故事。

---

### Emoji 的历史简介：

*   **起源：** Emoji 最初于1999年由日本电信公司**NTT DoCoMo**的员工**栗田穰崇** 创作。日文“絵文字”（えもじ）这个词的意思就是“图片”（e） + “文字”（moji）。

*   **走向全球：** 随着苹果公司在2011年将Emoji键盘加入iOS系统，Emoji开始在全球范围内流行起来。

*   **Unicode 收录：** 为了让不同系统间能正确交换Emoji信息，Unicode联盟开始将流行的Emoji收录进Unicode标准。这是Emoji能够全球通用的技术基础。

## 如何使用

在 Markdown 中，Emoji 有两种使用方式：
...
```markdown
使用冒号语法：:smile: :rocket: :tada:
```

前提你的markdown解析器必须支持这种代码形式。

如果你使用的是MemE主题，要把`enableEmoji = false`改成`true`。

## emoji速查表

我从网上搜罗了一些emoji表情和代码的使用方式，可以参照使用，浏览器可以按ctrl+f查找使用。
